...
export default function({
  ...
  return {
    pre() {
      ...
      this.orStatementRegex = new RegExp(this.opts.orStatementRegexString || 'or');
      this.controllerNameRegex = new RegExp(this.opts.controllerNameRegexString || 'router');
      this.requireScopesRegex = new RegExp(this.opts.requireScopesRegexString || 'requireScopes');
    },
    visitor: {
      CallExpression(path): void {
        if (
          ...
          this.controllerNameRegex.test(path.node.callee.object.name) && // TODO: Make regex
          Object.values(RequestMethod).includes(path.node.callee.property.name)
        ) {
          ...
          if (t.isCallExpression(path.node.arguments[1])) {
            ...
            if (
              ...
              orGroups.push(calculateOrGroup(t, arg.arguments[0]));
              e = true;
            }
            ...
            if (t.isMemberExpression(arg.callee)) {
              ...
              let depth = 0;
              while (e === false && depth < (this.opts.maxDepth || 3)) {

                if (isOrStatement(callee, this.orStatementRegex, t)) {
                  // is or statement
                  orGroups.push(
                    calculateOrGroup(t, callExpression.arguments[0])
                  );

                  if (t.isCallExpression(callee.object)) { 
                    callExpression = callee.object;
                    if (t.isMemberExpression(callee.object.callee)) {// Check if is chained Or e.g: requireScopes().or().or();
                    ...
                    throw Error("Callee Object was not Call Expression")
                  }
                }
                depth++;
              }
    ...
    post() {
      if (this.permissions.size > 0) {
        ...
        fs.writeFileSync(endPath, JSON.stringify(ServiceController, null, 2));
      }
    }
  };
}
